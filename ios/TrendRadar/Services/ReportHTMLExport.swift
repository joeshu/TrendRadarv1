...
struct ReportHTMLFormatter {
    func render(_ report: ReportDetail) -> String {
        ...
        </style></head><body><div class="page"><header class="hero"><h1>\(escape(report.title))</h1><div class="meta">TrendRadar · \(escape(report.type.displayName)) · 生成于 \(escape(date(report.generatedAt))) · 数据完整度：\(completeness)</div></header><div class="stats">\(stats)</div>\(topicStats)\(analysis)\(newItems)\(sections)\(failures)<div class="footer">报告 ID：\(report.id.uuidString) · 协议 v\(report.metadata.schemaVersion) · App \(escape(report.metadata.appVersion)) · 时区 \(escape(report.metadata.timeZone)) · 共 \(report.sections.flatMap(\.items).count) 条</div></div></body></html>
        """
    }

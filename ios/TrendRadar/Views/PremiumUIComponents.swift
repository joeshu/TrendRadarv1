...
struct PremiumTag: View {
    ...
    var body: some View { Text(text).font(AppTheme.metadataFont).foregroundStyle(tint).padding(.horizontal, 8).padding(.vertical, 5).background(tint.opacity(0.12)).clipShape(Capsule()) }
}
        ...
        HStack(alignment: .bottom, spacing: 14) {
            VStack(alignment: .leading, spacing: 6) {
                ...
                Text(subtitle)
                    ...
                    .foregroundStyle(assetName == nil ? AppTheme.textSecondary : Color.white.opacity(0.82))

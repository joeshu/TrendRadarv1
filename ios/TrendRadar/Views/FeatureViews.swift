        ...
        Task {
            ...
            if let generated = await reportStore.latestGeneratedDetail() {
                ...
                if let failure = generated.aiAnalysis?.failureMessage, !failure.isEmpty {
                    actionFeedback = .failure("报告已保存，但 AI 分析失败：\(failure)")
                } else if generated.aiAnalysis?.hasContent == true {
                    actionFeedback = .success("AI 洞察报告已生成并保存")
                } else {
                    actionFeedback = .failure("报告已保存，但没有生成可用的 AI 分析，请检查 AI 配置")
                }
            } else {
                await loadLatestAnalysis()
                actionFeedback = .failure(reportStore.errorMessage ?? "报告生成失败，请稍后重试")
            }
        }
    }
